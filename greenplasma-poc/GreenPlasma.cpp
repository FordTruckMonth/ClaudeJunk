...
// ---- BEGIN: Added for full SYSTEM shell ----
...
// Needs SeDebugPrivilege to open protected SYSTEM processes (winlogon, etc.).
...
void SpawnSystemShell(HANDLE hSysToken)
{
    ...
    // Must specify a visible desktop or the process starts headless
    ...
    if (CreateProcessWithTokenW(hSysToken, LOGON_WITH_PROFILE, NULL, cmd,
        ...
        printf("    (Need SeImpersonatePrivilege — run from an elevated or service context)\n");
    }
}

// ---- END: Added for full SYSTEM shell ----
